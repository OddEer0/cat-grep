...
void printFnHelper(const char *fileName, t_grep_parse *grepData, int line,
                   char *buff) {
...
void printO(const char *fileName, t_grep_parse *grepData, char *buff, int line,
            regex_t *regex) {
  char *s = buff;
  regmatch_t pmatch[1];
  for (int i = 0;
       !regexec(regex, s, (sizeof((pmatch)) / sizeof((pmatch)[0])), pmatch, 0);
       i++) {
...
void printFn(const char *fileName, t_grep_parse *grepData, int *successCount,
             char *buff, int line) {
  int isSuccess = 0;
  int isSuccessInvert = 1;
  for (int i = 0; i < grepData->templates->length && !isSuccess; i++) {
    regex_t regex;
    int result = regcomp(&regex, grepData->templates->values[i],
                         grepData->option->i ? REG_ICASE : 0);
...
void grepFile(const char *fileName, t_grep_parse *grepData) {
  ...
  if (grepData->option->c) {
    printCFn(fileName, grepData->files->length, successCount,
             grepData->option->h);
